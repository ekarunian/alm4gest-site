[日本語](/) | [English](/en/) | [한국어](/ko/) | [繁體中文](/zh/)

### ALM4GEST

An independently developed score management app for KONAMI's music games — "SOUND VOLTEX" and "DanceDanceRevolution" in the first release.

- Import your play data from e-amusement and browse scores, clear status, and EX scores in one list
- See what you are good at and where you fall short, through T-scores and tier lists built from all-player statistics
- Keep a record of your progress with growth charts and shareable images

This app is an unofficial tool developed and operated by an individual.
It is not provided or operated by Konami Arcade Games Co., Ltd. or any KONAMI group company, and has no affiliation with KONAMI.

#### Download

- App Store: [Download](https://apps.apple.com/app/id6793808647)
- Google Play: the Android app is in closed testing (not yet public)

#### Android testers wanted

The Android app is not public yet, so only testers can install it.
Play it and tell me about any bugs or issues you run into.

1. [Join the tester Google Group](https://groups.google.com/g/alm4gest_closed)
2. [Open the Google Play page](https://play.google.com/store/apps/details?id=com.alm4gest) and install the app

Do step 1 first.
The Play page will not open for an account that has not joined the group.
A Google account is required.

<!-- Android を一般公開（製品版リリース）したら、この節を削除し、
     上の "the Android app is in closed testing (not yet public)" を次のリンクへ差し替える:
     [Download](https://play.google.com/store/apps/details?id=com.alm4gest)
-->

#### Terms and policies

- [Terms of Service](./terms)
- [Privacy Policy](./privacy)
- [Data deletion request](./delete)

#### Contact

Operator: Takahiro Kawaguchi (@ekarunian)
Contact: [kawaguchi.ek@gmail.com](mailto:kawaguchi.ek@gmail.com)
