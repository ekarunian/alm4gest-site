[日本語](/) | [English](/en/) | [한국어](/ko/) | [繁體中文](/zh/)

### ALM4GEST

코나미의 음악 게임(초판은 "SOUND VOLTEX"와 "DanceDanceRevolution")의 스코어를 기록·분석하는, 개인이 개발한 스코어 관리 앱입니다.

- e-amusement의 플레이 데이터를 가져와 스코어·클리어 상황·EX 스코어를 한눈에 볼 수 있습니다
- 전체 플레이어 통계와 비교한 T점수·티어표로 잘하는 것과 약점을 알 수 있습니다
- 성장 그래프·공유 이미지로 실력 향상의 기록을 남길 수 있습니다

본 앱은 개인이 개발·운영하는 비공식 도구입니다.
주식회사 코나미 아케이드 게임스(Konami Arcade Games Co., Ltd.) 및 코나미 그룹 각사가 제공·운영하는 것이 아니며, 코나미와 일절 관계가 없습니다.

#### 다운로드

- App Store: [다운로드](https://apps.apple.com/app/id6793808647)
- Google Play: Android 버전은 비공개 테스트 중입니다 (일반 공개 전)

#### Android 버전 테스터 모집

Android 버전은 아직 일반 공개되지 않아 테스터로 참여한 분만 사용할 수 있습니다.
설치해서 플레이해 보시고, 발견한 버그나 불편한 점을 알려 주시기 바랍니다.

1. [테스터용 Google 그룹에 가입하기](https://groups.google.com/g/alm4gest_closed)
2. [Google Play 페이지](https://play.google.com/store/apps/details?id=com.alm4gest)를 열어 설치하기

1번을 먼저 진행해 주시기 바랍니다.
그룹에 가입하지 않은 계정으로는 2번 페이지가 열리지 않습니다.
참여하려면 Google 계정이 필요합니다.

<!-- Android を一般公開（製品版リリース）したら、この節を削除し、
     上の「Android 버전은 비공개 테스트 중입니다 (일반 공개 전)」を次のリンクへ差し替える:
     [다운로드](https://play.google.com/store/apps/details?id=com.alm4gest)
-->

#### 약관·정책

- [이용약관](./terms)
- [개인정보처리방침](./privacy)
- [데이터 삭제 신청](./delete)

#### 문의

운영자: 카와구치 타카히로(@ekarunian)
연락처: [kawaguchi.ek@gmail.com](mailto:kawaguchi.ek@gmail.com)
